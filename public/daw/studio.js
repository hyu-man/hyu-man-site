...
export const NAMES=['蟻','ライフ','コラッツ','そろう','歩幅'];
...
export function defaults(){return {version:1,title:'まだ名前のない曲',prompt:'',style:'glass',bpm:112,bars:32,key:57,space:.28,variation:0,tracks:NAMES.map((name,i)=>({seed:[0,93,27,15,5][i],vol:[.65,.55,.7,.35,.55][i],pan:[-.25,.3,0,-.1,.1][i],tone:['bell','pad','bass','percussion','percussion'][i],mute:false,solo:false,density:1,sections:[[.45,1,1,1,0,.45,1,.45],[1,1,.45,1,1,.45,1,1],[0,.45,1,1,.45,1,1,.45],[0,0,.45,1,0,.45,1,0],[0,.45,1,1,0,.45,1,0]][i]}))};}
export function validate(raw){
 if(!raw||raw.version!==1||!Array.isArray(raw.tracks)||raw.tracks.length!==5)throw Error('このスタジオのプロジェクトを選んでください。');
 const p=defaults();p.title=String(raw.title||p.title).slice(0,80);p.prompt=String(raw.prompt||'').slice(0,500);
 ...
 p.tracks=raw.tracks.map((t,i)=>{t=t||{};return {seed:Math.floor(clamp(t.seed,[0,0,1,2,1][i],[400,99999,9999999,60,15][i],p.tracks[i].seed)),vol:clamp(t.vol,0,1,.6),pan:clamp(t.pan,-1,1,0),density:clamp(t.density,.25,1,1),tone:['bell','pad','bass','pluck','percussion'].includes(t.tone)?t.tone:p.tracks[i].tone,mute:!!t.mute,solo:!!t.solo,sections:Array.from({length:8},(_,j)=>clamp(t.sections?.[j],0,1,1))};});return p;
}
export function fromWords(p){
 ...
 const h=hash(text+'|'+n.variation);n.tracks.forEach((t,i)=>{t.seed=[h%401,(h>>>4)%100000,1+(h>>>8)%5000,6+(h>>>16)%35,3+(h>>>23)%7][i];});
 return n;
}
...
export function compose(p){
 ...
 for(let k=0;k<total;k++){
  ...
  for(let i=0;i<5;i++){
   ...
   else {const hit=(k%16*tr.seed)%16<tr.seed;if(hit)raw=[{drum:k%4===0?'kick':'hat'}];}
   const activity=tr.sections[section];
   if(tr.mute||(solo&&!tr.solo)||activity===0)continue;
   for(let j=0;j<raw.length;j++){
    const e=raw[j];const divisor=p.style==='night'?[8,3,4,2,4][i]:[2,1,2,1,1][i];
    ...
    if(e.drum){dur=e.drum==='shot'?.35:.1;amp=e.drum==='shot'?.17:.09;}
     ...
     if(p.style==='night')dur*=1.5;amp=(e.vol||.06)*(kind==='pad'?.9:1.6);
    }
    const t=k*step;const endFade=Math.min(1,(total-k)/(span*.5));
    evs.push({i,k,t,midi,hz:midi===undefined?0:hz(midi),dur,amp:amp*activity*endFade,kind,drum:e.drum});
   }
  }
 }
...
export function midi(p){
 ...
 const tracks=p.tracks.map((tr,i)=>{const channel=i>2?9:i,notes=[];
  ...
  notes.sort((a,b)=>a[0]-b[0]||a[1][0]-b[1][0]);let prev=0,bytes=[0,192|channel,[10,89,38,0,0][i]];
  for(const [t,data]of notes){bytes.push(...vlq(t-prev),...data);prev=t;}return chunk('MTrk',[...bytes,0,255,47,0]);});
 return new Blob([new Uint8Array([...chunk('MThd',[0,1,0,6,1,224]),...tempo,...tracks.flat()])],{type:'audio/midi'});
}
