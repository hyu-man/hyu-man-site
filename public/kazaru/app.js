...
const director=new LocalDirector();let automatic=false,mood='cute';
const sceneNames={listen:'音を聴いています…',quiet:'そっと、ひとつ。',rise:'ふくらんで、きらきら。',flow:'音にのって、ゆらゆら。'};
function autoUI(){$('auto').setAttribute('aria-pressed',String(automatic));$('auto').textContent=automatic?'おまかせ中 ♡（押すと手動）':'おまかせ演出 ♡';if(!automatic)$('scene').textContent='手動で、好きなかざりを。'}
$('auto').onclick=()=>{automatic=!automatic;director.reset();clear();autoUI();if(automatic){$('scene').textContent=sceneNames.listen;play()}};
for(const b of document.querySelectorAll('[data-mood]'))b.onclick=()=>{mood=b.dataset.mood;director.reset();clear();for(const x of document.querySelectorAll('[data-mood]'))x.setAttribute('aria-pressed',String(x===b));if(automatic)$('scene').textContent=sceneNames.listen};
...
function clear(){director.reset();particles=[];envelope=0;average=.015;spawnClock=0;ctx.clearRect(0,0,width,height);$('meter').style.width='0%'}
...
function resetControls(){loaded=false;$('auto').disabled=true;$('play').disabled=true;$('back').disabled=true;$('seek').disabled=true;$('play').textContent='▶ 再生';$('seek').value=0;$('time').textContent='0:00 / 0:00'}
...
video.addEventListener('loadedmetadata',()=>{if(!Number.isFinite(video.duration)||video.duration<=0){status('この動画の長さを読み取れませんでした。別の動画を試してね。');return}loaded=true;$('auto').disabled=false;$('play').disabled=false;$('back').disabled=false;$('seek').disabled=false;updateTime();status('準備できたよ。再生して、音でかざろう。')});
...
$('play').onclick=()=>{if(video.paused)play();else video.pause()};$('back').onclick=()=>{if(!loaded)return;video.currentTime=0;clear();play()};video.addEventListener('play',()=>{$('play').textContent='Ⅱ 一時停止'});video.addEventListener('pause',()=>{$('play').textContent='▶ 再生';status('一時停止中。');$('meter').style.width='0%'});video.addEventListener('ended',()=>{$('play').textContent='▶ もう一度';status('もう一度、ちがう気分で？')});
...
for(const button of document.querySelectorAll('[data-mode]'))button.onclick=()=>{automatic=false;autoUI();mode=button.dataset.mode;for(const b of document.querySelectorAll('[data-mode]'))b.setAttribute('aria-pressed',String(b===button));clear()};
...
const plan=automatic?director.step(rms,dt,mood):null;if(plan){mode=plan.kind;const label=sceneNames[plan.state];if($('scene').textContent!==label)$('scene').textContent=label;for(const b of document.querySelectorAll('[data-mode]'))b.setAttribute('aria-pressed',String(b.dataset.mode===mode))}
if(enabled&&amount>0){spawnClock+=dt;const strength=amount*(plan?plan.scale:1);const rate=mode==='sleep'?(1+envelope*5):(2+envelope*30);if(rms>.002&&spawnClock>1/(rate*strength)){spawnClock=0;emit(1+Math.floor(strength*2))}const hit=plan?plan.burst:(mode!=='sleep'&&rms>.035&&rms>average*1.32&&elapsed-lastHit>.28);if(hit){lastHit=elapsed;emit(Math.round(7+strength*30),true)}}}
