...
function ring(t){for(let j=0;j<8;j++){let z=(j/8+t*.28)%1,r=20+z*z*600;d.globalAlpha=.18+z*.65;for(let i=0;i<14;i++){let a=i*TAU/14+t*.22+j*.08;heart(480+Math.cos(a)*r,255+Math.sin(a)*r*.62,1+z*4)}}d.globalAlpha=1}
...
if(!reduced.matches)for(let start of [42+2/30,57+20/30,65+10/30,91+18/30,104+8/30,121+29/30]){let u=(time-start)/1.1;if(u>=0&&u<1)for(let j=0;j<32;j++){let a=j*TAU/32,r=30+u*660;heart(480+Math.cos(a)*r,250+Math.sin(a)*r*.65,1+(1-u)*4,j%2?K:G)}}
...
touches=touches.filter(q=>now-q.time<1400);for(let q of touches){let age=(now-q.time)/1400;d.globalAlpha=1-age;for(let i=0;i<12;i++){let a=i*TAU/12;heart(q.x+Math.cos(a)*age*130,q.y+Math.sin(a)*age*100,2+(1-age)*2,i%2?K:G)}}d.globalAlpha=1;
}requestAnimationFrame(paint)}
c.addEventListener('pointerdown',e=>{if(reduced.matches)return;let r=c.getBoundingClientRect();touches.push({x:(e.clientX-r.left)/r.width*960,y:(e.clientY-r.top)/r.height*540,time:performance.now()});touches=touches.slice(-12)});
